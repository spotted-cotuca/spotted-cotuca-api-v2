...
namespace SpottedCotuca.Application.Tests.Repositories
{
    ...
    public class DatastoreSpotsRepositoryReadTests : DatastoreSpotRepositoryTests
    {
        ...
        public void ShouldReadTheSpots()
        {
            ...
            responseSpots.Count.Should().Be(fetchedSpots.Count);

            responseSpots.FirstOrDefault().Id.Should().Be(fetchedSpots.FirstOrDefault().Id);
            responseSpots.FirstOrDefault().Message.Should().Be(fetchedSpots.FirstOrDefault().Message);
            responseSpots.FirstOrDefault().Status.Should().Be(fetchedSpots.FirstOrDefault().Status);
            responseSpots.FirstOrDefault().PostDate.ToString("dd/MM/yyyy HH:mm").Should()
                .Be(fetchedSpots.FirstOrDefault().PostDate.ToString("dd/MM/yyyy HH:mm"));
            responseSpots.FirstOrDefault().FacebookId.Should().Be(fetchedSpots.FirstOrDefault().FacebookId);
            responseSpots.FirstOrDefault().TwitterId.Should().Be(fetchedSpots.FirstOrDefault().TwitterId);
        }
        ...
        new public void Cleanup()
        {
            ...
            {
                _provider.Db.Delete(spot.Id.ToSpotKey());
            }
        }
